...
<form:form id="experiment" modelAttribute="experiment" action="${ctx}/save" method="post" target="hidden_frame_add">
    ...
    <form:hidden id="knownPoint"  path="expParameter.knownPoint"/>
    <form:hidden id="unKnownPoint" path="expParameter.unKnownPoint"/>
    ...
    <div id="center">
        <div id="chooseUploadData" class="easyui-window" title="选择上传文件" data-options="modal:true,iconCls:'icon-save',closed:true,${windowsProperties}" style="overflow:hidden;width:366px;height:273px;padding:10px;">
            ...
            <div id="buttons" style="" class="button-arrange">
        ...
        <table id="expTableforPara" cellpadding="5" class="expTableforPara" align="center">
            <tr>
                <td>
                    <label>选择数据： </label>
                </td>
                <td>
                    ...
                    <div id="textDiv" style="">
                        <label>已知测点数据：<div id = 'kText' style="display: table-row"></div></label>
                        <br/>
                        <div style="margin-top: 5px">
                            <label>未知测点数据：<div id = 'uText' style="display: table-row"></div></label>
                        </div>
            ...
            <tr>
                <td colspan="2" align="center">
                    <div style="">
                        ...
                        <a id="checkData" href="javascript:void(0)"  style="color: #439eb7;margin-left: 50px;" class="easyui-linkbutton" onclick="$('#resultData').dialog('open');" text="查看结果数据"></a>
                    </div>
                </td>
            </tr>
        </table>
        ...
        */
        ...
        $(document).ready(function(){
            ...
            if(dataId!=null&&dataId.length!==0){
                $('#kText').html($('#knownPoint').val());
                $('#uText').html($('#unKnownPoint').val());
        ...
        $("#resultPic").dialog({
            ...
            onOpen:function(){
                ...
                $('#img').attr("src",'${ctx}/getImg?expId='+expId +'.'+Math.random());
            }
        });
        ...
        function runAlgorithms(){
            ...
            socket.onopen = function(event){
                var str = 'expClass='+expClass+'&freqRangeMin='+param1+'&freqRangeMax='+param2+'&samplingFreq='+param3+'&p2Predict='+param4+'&expId='+expId+'&dataId='+dataId+'&algorithm='+algorithm;
                socket.send(str);
                //清空流动文本
                textflow=[];
            };
        ...
        $('#buttonSave').click(function(){
            ...
            var str = ['knownPoint=Tdata', 'unKnownPoint=Pdata'];
            data.append(str[0], file1[0]);data.append(str[1], file2[0]);
            //将文件名存于参数属性
            $('#knownPoint').val(file1[0].name);$('#unKnownPoint').val(file2[0].name);

            var dataId = $('#dataId').val();
            var url = '${ctx}/uploadData' + '?param1=' + str[0] +
            '&param2='+ str[1] + '&expId=' + $('#expId').val()+ '&expClass=' + $('#expClass').val();
            if(dataId!==undefined&&dataId.length!==0){
