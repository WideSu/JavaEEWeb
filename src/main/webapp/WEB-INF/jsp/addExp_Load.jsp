...
<form:form id="experiment" modelAttribute="experiment" action="${ctx}/save" method="post" target="hidden_frame">
    ...
    <!-- 上传响应信号数据和源信号数据 默认隐藏，点击上传数据后开启-->
        <div id="LR_chooseUpload" class="easyui-window" title="选择上传文件" data-options="modal:true,iconCls:'icon-save',closed:true,${windowsProperties}" style="overflow:hidden;width:366px;height:273px;padding:10px;">
            ...
            <div id="buttons" style="align:center" class="button-arrange">
                ...
                <button id="buttonClose" type="button" class="easyui-linkbutton" onclick="$('#LR_chooseUpload').window('close')">取消</button>
            </div>
        </div>
        
        <!-- 查看试验结果图 使用datagrid加载多张图片，默认隐藏，点击查看图片后开启 -->
        <div id="LR_resultPic" class="easyui-dialog" title="试验结果图" data-options="modal:true,iconCls:'icon-save',closed:true,closable:true" style="width:720px;height:580px;padding:10px">
				<table id="LR_grid" >
		...
		<!-- 选择已上传文件列表 -->
		<div id="LR_chooseUploadedData" class="easyui-window" title="选择已上传文件" data-options="modal:true,iconCls:'icon-save',closed:true,${windowsProperties}" style="width:600px;height:380px;padding:10px;">
            ...
            <div align="right" style="margin-top: 1%">
                ...
                <a href="javascript:void(0)" class="easyui-linkbutton"  onclick="$('#LR_chooseUploadedData').window('close')" text="取消"></a>
            </div>
        </div>
...
　　				var tr = panel.find('div.datagrid-body tr');
    ...
    //将获取到的src传给后台输出图片
        ...
        function changePic(index){
            //获取该行的数据
            var data = $('#LR_grid').datagrid("getRows")[index];
            //获取该行图片的url
            var oldUrl = data.url;
	...
	<!-- 中心页面，默认开启，不可隐藏 -->
     <table id="expTableforPara" cellpadding="5" class="expTableforPara">
            ...
            <tr>
                ...
                <td>
                    <label><form:input id="freqRangeMin" cssClass="textbox" path="expParameter.freqRangeMin" onkeyup="onlyNum(this)" name="param" cssStyle="width: 15%;height:25px;font-size: 15px"/>Hz</label>
                    <label style="margin-left: 20px;padding-right: 20px">-</label>
                    <label><form:input id="freqRangeMax" cssClass="textbox" path="expParameter.freqRangeMax" onkeyup="onlyNum(this)" name="param" cssStyle="width: 15%;height:25px;font-size: 15px"/>Hz</label>
                </td>
            </tr>
            <tr>
    ...
    //打开查看对比图的div 并且重新加载datagrid
    	function opendialog(){
    	$("#LR_grid").datagrid({url:"${ctx}/getMultipleImgURL?expId="+$("#expId").val()});
    	$("#LR_grid").datagrid("reload");
    	$('#LR_resultPic').dialog('open');
    	}
        ...
        function chooseFiles(){
            ...
            if(!flag){
                ...
                if($('#uploadNew')[0].checked){
                    $('#LR_chooseUpload').window('open');
                }else if($('#chooseUsed')[0].checked){
                    $('#LR_chooseUploadedData').window('open');
                    openDataTable();
                }
            }
    ...
    function validDateFileNew(self,spanName){
        ...
        function showUploaded(selectedRow){
            ...
            $('#LR_chooseUploadedData').window('close');
            $('#radioDiv1').css('display','none');
            $('#radioDiv2').css('display','none');
            $('#textDiv').css('display','block');
